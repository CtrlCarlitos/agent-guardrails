...
func TestGenConfigCodexGolden(t *testing.T) {
	bin := buildBinary(t) // from contract_test.go, same package
	cmd := exec.Command(bin, "gen-config", "codex", "--print", "--binary", "/usr/local/bin/guardrail")
	var out bytes.Buffer
	cmd.Stdout = &out
	if err := cmd.Run(); err != nil {
		t.Fatalf("run: %v", err)
	}
	golden := "fixtures/codex/hooks.golden.json"
	if *updateGolden {
		if err := os.WriteFile(golden, out.Bytes(), 0o644); err != nil {
			t.Fatal(err)
		}
