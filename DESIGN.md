...
| # | Policy | Notes |
|---|---|---|
| P1 | Destructive-command gate (Bash PreToolUse) | keep `rm -rf` outside safe roots, `git push -f`, `git clean -f`, docker teardown/prune, `$()`-computed / cross-worktree `docker rm`. Recursive/forced `rm` and output redirects allow strict physical descendants of system temp on every plane and actual-home Claude project memory only on the Claude plane; the roots remain protected. Add `dd`/`mkfs`/`wipefs`/`shred` (deny); `truncate`/`:>`/`>` onto tracked file, `chmod -R`/`chmod 777`/`chown -R`, `find -delete` / `-exec rm`, `kill -9`/`killall`/`pkill` (ask); `sudo`/`su`/`doas` (deny) |
...
| P5 | Filesystem-scope + "runs-later" gate (Edit/Write/redirect) | writes outside the worktree root (ask), except native file-tool writes to strict physical system-temp descendants on every plane and actual-home Claude project-memory descendants on the Claude plane; those roots remain protected. Symlink-escape — path inside repo resolving outside → deny; CI config (`.github/workflows/**`, `.pre-commit-config.yaml`, …) ask; agent-self-config (`.claude/**`, `CLAUDE.md`, `AGENTS.md`, `.mcp.json`, `.envrc`, shell rc) deny; lockfile hand-edits, `Dockerfile`/`Makefile`/`*.tf`/`conftest.py`/`setup.py` (ask) |
