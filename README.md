...
```sh
# 1. Download the binary for your platform and verify it.
ver=v0.20.27-dev
curl -fsSLO "https://github.com/CtrlCarlitos/agent-guardrails/releases/download/$ver/guardrail_linux_amd64"
curl -fsSLO "https://github.com/CtrlCarlitos/agent-guardrails/releases/download/$ver/SHA256SUMS"
sha256sum --check --ignore-missing SHA256SUMS
install -m 0755 guardrail_linux_amd64 ~/.local/bin/guardrail

# 2. Wire it into each agent host you use (idempotent; re-run any time).
...
It is a static guard on tool calls, not a sandbox: it inspects what the agent *asks* to run. What a process does after it's allowed to start is out of scope, on purpose ([ADR-0012](./docs/adr/0012-static-analysis-boundary-and-shape-threshold.md)).
...
**Per project — an Overlay.** Commit a `guardrail.toml` ([example](./guardrail.toml.example)). It can add rules, extend the safe roots and secret tiers, and *request* loosenings:
