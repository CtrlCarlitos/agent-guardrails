...
`
	...
	for i, want := range wantPaths {
		...
		if !reflect.DeepEqual(envelope["paths"], []any{want}) {
			t.Errorf("envelope %d paths projection = %#v, want [%q]", i, envelope["paths"], want)
		}
	}
	if got := envelopes[7]["url"]; got != "https://example.test/docs" {
		t.Errorf("webfetch URL projection = %#v", got)
	}
	if _, ok := envelopes[8]["paths"]; ok {
		t.Errorf("apply_patch was broadened into a paths projection: %#v", envelopes[8])
	}
	custom := envelopes[9]
...
`
	...
	wantAsk := "guardrail: Operator authorization required: external egress needs approval. Request authorization for this exact action: bash true. If the operator approves, retry this exact tool call within 10 minutes. If the authorization expires, stop and wait for the operator to return — say what you were doing and that approval expired; do not keep retrying. Do not alter or broaden the action."
	for _, tt := range []struct {
		response string
		wantOut  string
		wantCode int
