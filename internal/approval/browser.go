...
func browserHandler(browser *Browser) http.Handler {
	page := template.Must(template.New("approval").Parse(`<!doctype html><title>Guardrail approval</title><h1>Guardrail approval</h1><p><strong>Guardrail instance:</strong> {{.Instance}}. Approve with the authenticator you enrolled for <em>this</em> instance.</p><p>Passkeys for localhost live only on the device that created them. If your browser offers a phone, iPhone or Android option (a QR code), do not use it unless you enrolled this instance's authenticator on that phone: a phone cannot approve with a passkey it does not hold, and will say "No passkeys available".</p><dl><dt>Request</dt><dd>{{.RequestIDPrefix}}</dd><dt>Plane</dt><dd>{{.Plane}}</dd><dt>Repository</dt><dd>{{.RepoRoot}}</dd><dt>Host</dt><dd>{{.Host}}</dd><dt>Action</dt><dd>{{.Action}}</dd>{{if .Summary}}<dt>Details</dt><dd>{{.Summary}}</dd>{{end}}<dt>Scope</dt><dd>{{.Scope}}</dd><dt>Expires</dt><dd>{{.ExpiresAt}}</dd></dl><output id="status">Waiting for WebAuthn assertion</output><script>const publicKey = {{.Options}};const decode = value => Uint8Array.from(atob(value.replace(/-/g,"+").replace(/_/g,"/")), c => c.charCodeAt(0));const encode = value => btoa(String.fromCharCode(...new Uint8Array(value))).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"");publicKey.challenge = decode(publicKey.challenge);publicKey.allowCredentials = (publicKey.allowCredentials || []).map(credential => ({...credential, id: decode(credential.id)}));const status = document.getElementById("status");window.requestWebAuthnAssertion = () => navigator.credentials.get({publicKey}).then(credential => fetch("/assertion", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:credential.id,rawId:encode(credential.rawId),type:credential.type,response:{authenticatorData:encode(credential.response.authenticatorData),clientDataJSON:encode(credential.response.clientDataJSON),signature:encode(credential.response.signature),userHandle:credential.response.userHandle && encode(credential.response.userHandle)}})})).then(async response => status.textContent=await response.text()).catch(error => status.textContent="WebAuthn authentication was not completed (" + error.name + "). No enrolled authenticator responded. Use the authenticator and browser you enrolled this guardrail instance with, not a phone unless you enrolled it there. If that authenticator is gone, run guardrail operator recover-reset and then guardrail operator enroll from this instance's terminal, and choose this device when the system dialog asks.");window.requestWebAuthnAssertion();</script>`))
