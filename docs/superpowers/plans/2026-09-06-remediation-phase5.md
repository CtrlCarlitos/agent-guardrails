...
**Friction findings from the first 25 minutes of `v0.13.0-dev` in production (2026-09-07, 138 audit entries: 0 real denies, 17 asks — 15 of them false positives). Each reproduced on the deployed binary:**

- **NF-4 — `/dev/null` is treated as an out-of-repo write.** `ls x 2>/dev/null` → `P1.redirect` ask, "redirection onto a path outside the repo/safe roots: /dev/null". Six of the seventeen asks. Fix: redirects to `/dev/null`, `/dev/stdout`, `/dev/stderr`, `/dev/tty` are never write targets. Trivial; do first.
- **NF-5 — `P3.unresolved` fires on variables in non-path positions and on locally assigned literals.** `echo "exit: $?"`, `for f in a b; do echo "=== $f"; done`, and `SDK="/abs/lit"; grep -rn Foo "$SDK/api/"` all ask. P3 is a fail-closed backstop and stays unwaivable, but it should only fire when an unresolved word sits in a *path-bearing position* (operand of a mutating command, redirect target, path operand) — `$?` inside `echo` can never name a target — and a same-command `VAR=literal` assignment should be substituted before judging, exactly as Phase 2 tracks `cd`. `SCRATCH=/tmp/x; rm -rf "$SCRATCH/y"` must still ask, but under P1 for the resolved target, not P3. Ten of the seventeen asks.
- **NF-6 — the plane's own session scratchpad is "outside the repo".** `echo hi > /tmp/claude-1000/<project>/<session>/scratchpad/out.txt` → `P1.redirect` ask. Claude Code hands every session that directory for exactly this purpose. Fix: the engine recognizes the harness scratchpad for the current session (derivable from the payload's session id and the `/tmp/claude-<uid>/` convention) as a safe redirect root; `rm -rf` inside it remains governed by P1's safe-roots rule as today.

These are in scope for Phase 5 as a fourth task ("Task 4: production friction, NF-4/5/6"), after H-10/H-6/M-7 or before them at the executor's judgment — NF-4 alone removes a third of the asks and is a one-line change.
