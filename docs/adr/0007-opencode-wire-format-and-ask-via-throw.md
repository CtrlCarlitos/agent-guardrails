...
2. **An engine `ask` verdict also throws** from the plugin. The Adapter now
   emits the Engine-adapter shared operator-authorization guidance described in
   `2026-09-15-actionable-authorization-design.md`; a Deny cannot be
   authorized. The plugin is defense-in-depth on top of the declarative
   `permission` floor (generated by
