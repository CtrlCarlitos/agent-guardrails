...
| **CR-6 Force-push and remote-branch-delete via refspec** | **Fixed** | Phase 2; `v0.12.0-dev` planned, not published | Push argument parsing separates options, values, repository, and refspecs; `--repo` itself satisfies the repository operand, while leading `+`, empty-source deletion, and destination-side protected branches receive the required Verdicts ([`rules_git.go`](../../internal/engine/rules_git.go)). Attached/split repository-option, refspec, ordering, and parser-boundary locks are in [`rules_git_test.go`](../../internal/engine/rules_git_test.go). | - |
...
to describe logical-repository path rewriting. The resulting adversarial corpus
is **190 cases: 42 allow, 12 ask, and 136 deny**. Its harness validates the plane response and matching audit record
...
- The adversarial corpus is exactly **190 cases: 42 allow, 12 ask, 136 deny**.
