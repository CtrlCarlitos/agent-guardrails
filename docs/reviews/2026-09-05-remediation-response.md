...
| **CR-6 Force-push and remote-branch-delete via refspec** | **Fixed** | Phase 2; `v0.12.0-dev` planned, not published | Push argument parsing collects option and operand facts before classifying a positional repository; an explicit `--repo` anywhere before `--` makes every positional operand a refspec. Leading `+`, empty-source deletion, and destination-side protected branches therefore receive the required Verdicts regardless of option ordering ([`rules_git.go`](../../internal/engine/rules_git.go)). Attached, split, abbreviated, before/after-refspec, end-of-options, positional-repository, and benign controls are in [`rules_git_test.go`](../../internal/engine/rules_git_test.go). | - |
...
to describe logical-repository path rewriting. The exceptional CR-6 ordering
fix preserves the first 190 entries exactly and appends five locks. The resulting
adversarial corpus is **195 cases: 43 allow, 14 ask, and 138 deny**. Its harness validates the plane response and matching audit record
