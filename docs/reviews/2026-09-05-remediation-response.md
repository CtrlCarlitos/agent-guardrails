...
| **CR-6 Force-push and remote-branch-delete via refspec** | **Fixed** | Phase 2; `v0.12.0-dev` planned, not published | Push argument parsing separates options, values, repository, and refspecs; leading `+`, empty-source deletion, and destination-side protected branches receive the required Verdicts ([`rules_git.go:83-128`](../../internal/engine/rules_git.go#L83-L128)). Refspec and parser-boundary locks are in [`rules_git_test.go:131-210`](../../internal/engine/rules_git_test.go#L131-L210). | - |
...
| **CR-13 `docker compose -f` and the whole prune family** | **Fixed** | Phase 2; `v0.12.0-dev` planned, not published | Docker-family option parsing derives command-word chains for Docker, docker-compose, Podman, and nerdctl before matching compose down and prune families ([`rules_bash.go:633-682`](../../internal/engine/rules_bash.go#L633-L682)); `runnerInner` consumes run/exec values before exposing inner argv ([`tokenize.go:2472-2507`](../../internal/engine/tokenize.go#L2472-L2507)). Global, Compose, alternate-frontend, prune, and run/exec locks are in [`rules_bash_test.go:555-624`](../../internal/engine/rules_bash_test.go#L555-L624) and [`tokenize_test.go:914-978`](../../internal/engine/tokenize_test.go#L914-L978). | - |
...
| **H-4 Uncovered destructive primitives** | **Fixed** | Phase 2; `v0.12.0-dev` planned, not published | Option-aware destination checks cover mv/cp/ln/tee/install and deletion-mode rsync, including move sources and remote destinations ([`rules_bash.go:64-108`](../../internal/engine/rules_bash.go#L64-L108), [lines 164-238](../../internal/engine/rules_bash.go#L164-L238), [`rules_path.go:260-428`](../../internal/engine/rules_path.go#L260-L428)). Find destructive exec families are handled at [`rules_bash.go:727-737`](../../internal/engine/rules_bash.go#L727-L737); update-ref, worktree remove, switch discard, and git rm at [`rules_git.go:67-82`](../../internal/engine/rules_git.go#L67-L82); and option-aware SSH remote commands remain subject to host and nested Bash checks. Destination, rsync, find, git, and SSH locks are in [`rules_bash_test.go:696-1014`](../../internal/engine/rules_bash_test.go#L696-L1014), [`rules_git_test.go:56-129`](../../internal/engine/rules_git_test.go#L56-L129), and [`rules_net_test.go:520-684`](../../internal/engine/rules_net_test.go#L520-L684). | -; CR-9's unlisted secret readers and H-10's unknown tools remain separate Phase 4 gaps. |
...
prefix. The resulting adversarial corpus is **178 cases: 38 allow, 11 ask, and
129 deny**. Its harness validates the plane response and matching audit record
...
- The adversarial corpus is exactly **178 cases: 38 allow, 11 ask, 129 deny**.
