...
allows the requested `P6.egress` Waiver, secret exception, and audit redirect.
The `safe_roots` entry needs no grant but still must resolve inside the
repository. Keep top-level Overlay fields such as `audit_log` and `waive`
before `[slots]`; placing them after that header would make them slot fields in
TOML.
